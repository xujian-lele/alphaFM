...
-cn \<column_name\>: txt格式样本的列名以“,”拼接起来的字符串，isf为txt时生效, 必须设置!default:空字符串<br>
-cs \<combine_schema\>: 特征序列以“,”拼接的字符串，isf为txt时生效, 必须设置!default:空字符串<br>
-pof \<predict_output_format\>: 设置为only_label_and_score时输出为"label+空格+score",default时输出为"输入样本+拼接符+score"，输入样本类型[-isf]txt时拼接符为\002,libsvm时为空格<br>
...
* 添加样本处理时的异常操作。
* 内存优化，提前预分配内存，对vector操作采用更高效算子;column_index设置为静态变量。
* 线上推理部分编写。